...
import { EspecificidadeVisaoGeral } from '../../../../../../shared/models/estudante-visao-geral.model';
...
export class EspecificidadeModalComponent implements OnInit {
  ...
  getPlaceholderDescricao(): string {
    ...
    switch (tipo) {
      case 'GATILHO_CRISE': return 'Ex: Sons altos e inesperados';
      case 'COMPORTAMENTO_ATIPICO': return 'Ex: Dificuldade em manter contato visual';
      case 'RESTRICAO': return 'Ex: Redirecionamento para ambiente tranquilo';
      default: return 'Ex: Sons altos e inesperados';
    }
  }
  ...
  excluir(id: number) {
    ...
    this.estudantesService.deleteEspecificidade(this.estudanteId, id).subscribe({
      next: () => {
        this.loading = false;
        this.salvo.emit(); // Força o refresh da tela
      },
      error: (err: any) => {
        this.loading = false;
  ...
  salvar() {
    ...
    const request$ = this.editingEspecificidadeId
      ? this.estudantesService.updateEspecificidade(this.estudanteId, this.editingEspecificidadeId, payload)
      : this.estudantesService.saveEspecificidade(this.estudanteId, payload);

    request$.subscribe({
      next: () => {
        this.loading = false;
        this.initForm(); // Limpa o form após salvar
        this.salvo.emit(); // Emite para recarregar da API
      },
      error: (err: any) => {
        this.loading = false;
