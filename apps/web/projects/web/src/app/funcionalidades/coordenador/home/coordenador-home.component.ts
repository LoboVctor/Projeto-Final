...
export class CoordenadorHomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registrosService
      ...
      .subscribe({ next: (dados) => this.registrosPendentes.set(dados), error: () => {} });
  ...
  abrirDetalhesAluno(estudante: EstudanteResumo): void {
    ...
    const dadosParaModal: AlunoModalData = {
      ...
      foto:
        estudante.foto ||
        `https://ui-avatars.com/api/?name=${estudante.nomeCompleto}&background=F0E6FF&color=4A148C`,
    };
