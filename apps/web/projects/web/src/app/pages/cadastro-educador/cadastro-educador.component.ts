...
import { CadastroLayoutComponent } from '../../shared/components/cadastro-layout/cadastro-layout.component';
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, NgIf, CadastroLayoutComponent],
  templateUrl: './cadastro-educador.component.html',
})
export class CadastroEducadorComponent {
  form: FormGroup;
  isLoading = false;

  constructor(private fb: FormBuilder, private cadastroService: CadastroService, private feedbackService: FeedbackService) {
    this.form = this.fb.group({
      nome: ['', [Validators.required, Validators.minLength(3)]],
      cpf: ['', [Validators.required, Validators.minLength(14), Validators.maxLength(14)]],
      especialidade: ['', [Validators.required, CustomValidators.notOnlyNumbers()]],
    ...
    if (this.form.valid) {
      ...
      this.cadastroService.submeterCadastro(payload).subscribe({
        next: () => {
          ...
          this.feedbackService.showSuccess('Cadastro de Educador realizado com sucesso!');
          this.form.reset();
        },
        error: () => {
          this.isLoading = false;
