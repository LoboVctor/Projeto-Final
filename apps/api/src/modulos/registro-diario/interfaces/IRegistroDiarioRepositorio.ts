...
export interface IRegistroDiarioRepositorio {
  criar(dados: Prisma.RegistroDiarioUncheckedCreateInput): Promise<RegistroDiario>;
  buscarTodos(): Promise<RegistroDiario[]>;
  buscarAlertasDiasAnteriores(educadorId: string): Promise<RegistroDiario[]>;
  contarRegistrosPreenchidos(educadorId: string, dataInicio: Date, dataFim: Date): Promise<number>;
  contarRegistrosEsperados(educadorId: string, dataInicio: Date, dataFim: Date): Promise<number>;
  buscarPorId(id: string): Promise<RegistroDiario | null>;
  atualizar(id: string, dados: Prisma.RegistroDiarioUncheckedUpdateInput): Promise<RegistroDiario>;
